...
class AVSRDataLoader:
    ...
    def process_single_frame(self, frame, landmarks):
        """Process individual frames for streaming"""
        if self.modality in ["video", "audiovisual"]:
            # Process video frame
            processed_frame = self.video_transform(frame)
            # Mouth cropping would already be handled by video_process
            return processed_frame
        
        if self.modality == "audio":
            # Need to accumulate audio samples
            # Implement ring buffer for audio
            raise NotImplementedError("Audio-only streaming not implemented")
